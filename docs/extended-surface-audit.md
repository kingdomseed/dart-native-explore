...
| LOD | `render_extras_codecs.dart:124-308` `LodCodec` (`levels`+`screenSize`, `lodBias`, `hysteresis`, `blendRange`) | component; unhandled | `SCNGeometry.levelsOfDetail` + `SCNLevelOfDetail(screenSpaceRadius:)`; screenSize→px approx; no hysteresis/blend | none — per-frame camera test + geometry/entity swap; no blend | **partial** — iOS small, Android medium |
