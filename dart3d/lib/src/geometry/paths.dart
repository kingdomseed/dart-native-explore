/// Curve helpers for the path-driven dart3d geometry (W26).
///
/// A port of `flutter_scene-0.23.0`'s `scene_path.dart` — same
/// natural-parameter/arc-length duality, same rotation-minimizing
/// frames, same Catmull-Rom and polyline curves — so a [CatmullRomPath]
/// evaluated in Dart produces the same samples the native decoders
/// produce for `tube`/`ribbon` shapes and the navigation-route use
/// case.
library;
...
///
/// A `ScenePath` is an immutable value type. Subclasses
/// ([PolylinePath], [CatmullRomPath]) define the curve; this base class
/// adds arc-length measurement and rotation-minimizing frames on top.
///
...
/// evenly along the curve ([positionAtDistance], [frameAtDistance]);
/// use it when spacing must stay uniform around bends, such as for
/// dashes or a swept profile.
abstract class ScenePath {
  _PathTable? _table;
  _PathTable get _baked => _table ??= _bake();

  /// The point on the curve at natural parameter [t] (clamped to
  /// `0..1`).
  ...
  /// otherwise they are spaced by equal natural parameter. [count]
  ...
  /// Samples [stations] rotation-minimizing frames, spaced by equal arc
  /// length.
  List<ScenePathFrame> evenlySpacedFrames(int stations) {
    final total = length;
    return <ScenePathFrame>[
      for (var i = 0; i < stations; i++)
        frameAtDistance(stations == 1 ? 0.0 : total * i / (stations - 1)),
    ];
  }
...
class PolylinePath extends ScenePath {
  /// Creates a polyline through [points], which is copied. At least
