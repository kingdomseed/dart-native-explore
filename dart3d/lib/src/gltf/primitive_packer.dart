...
// `lib/src/importer/src/gltf/primitive_packer.dart`
// (MIT, copyright Brandon DeRosier). Adaptations: `sceneLog` is imported
// from `package:scene/src/log.dart` (dart3d cannot import the
// `package:scene/scene.dart` barrel — see scene_model.dart), and
// KHR_draco_mesh_compression throws instead of decoding (no Draco codec is
// ported).

import 'dart:math' show sqrt;
import 'dart:typed_data';

import 'package:scene/src/log.dart' show sceneLog;
...
///
/// The vertex layout matches dart3d's skinned/unskinned vertex contract:
///
...
PackedPrimitive packGltfPrimitive({
  ...
  // KHR_draco_mesh_compression is recognized (parse does not warn) but no
  // Draco codec is ported; refuse loudly rather than reading the undecoded
  // accessor tables.
  if (primitive.draco != null) {
    throw const FormatException(
      'KHR_draco_mesh_compression: Draco-compressed meshes are not supported '
      'by the dart3d runtime importer; re-export without compression',
    );
  }
