...
// `lib/src/importer/src/gltf/bounds_baker.dart`
// (MIT, copyright Brandon DeRosier). Adaptation: the Draco decode path is a
// FormatException — KHR_draco_mesh_compression is recognized but not
// supported by the dart3d runtime importer.
...
/// once joints animate.
///
/// Pure Dart (no `dart:ui`/GPU), so it runs anywhere the document layer does.
library;
...
class _PoseChannel {
  ...
  List<double> _readKeyframe(
    ...
    return [for (var i = 0; i < componentCount; i++) values[base + i]];
  }
}
...
List<AabbBounds> _computeJointInfluenceAabbs(
  ...
  // Draco-compressed primitives cannot be read: no codec is ported, and the
  // primitive packer rejects the same primitive for the same reason.
  if (prim.draco != null) {
  ...
  final bufferViews = doc.bufferViews;
  final data = bufferData;
