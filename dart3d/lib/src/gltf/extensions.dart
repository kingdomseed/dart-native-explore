// Ported from flutter_scene 0.23.0 `lib/src/importer/src/gltf/extensions.dart`
// (MIT, copyright Brandon DeRosier). `KHR_draco_mesh_compression` moves from
// recognized to planned: dart3d does not port upstream's Draco decoder, so a
// document requiring it fails this gate and a document merely using it warns
// then throws when the compressed primitive packs.
...
const Set<String> kPlannedGltfExtensions = {
  // Upstream decodes this; dart3d's port does not (decoder size). A document
  // listing it in extensionsRequired is refused here; one merely listing it
  // in extensionsUsed warns, then throws when a compressed primitive packs.
